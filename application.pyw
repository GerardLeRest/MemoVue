...
from frame_gauche import *
from frame_droite_haute import *
from frame_droite_basse import FrameDroiteBasse
...
class Application(tk.Tk):
    ...
    def __init__(self, frame_droite_basse = None) :
        """Constructeur de la fenêtre principale"""
        tk.Tk.__init__(self)   # constructeur de la classe parente
        self.FrameDrBa = FrameDroiteBasse(self)
        self.FrameDrHa = frame_droite_haute(self)
        self.FrameG = frame_gauche(self,self.FrameDrBa.listeEleves)
        ...
        # bouton info
        self.bout_info = tk.Button(self, text="Info")
        self.bout_info.grid(row=2,column=1,pady=2)
        self.bout_info.bind("<Button-1>",self.information)
    ...
    def actDesZonesSaisies(self) -> None:
        ...
        print("[DEBUG] Appel à actDesZonesSaisies")
        print("Mode:", self.FrameDrBa.selectPrenNom.get())  # "1", "2", "3"
    ...
    def configAutresModes(self) -> None:
        ...
        print("listeEleves FrameG: ",  self.FrameG.listeEleves) 
    ...
    def verifier(self) -> None:
        ...
        # Réponses attendues
        nom_attendu = self.FrameG.listeEleves[self.FrameG.rang + 1][1].strip()
        prenom_attendu = self.FrameG.listeEleves[self.FrameG.rang + 1][0].strip()
        ...
        self.FrameDrHa.canvas.itemconfig(self.FrameDrHa.photo_sur_canvas, image=self.photo)
    ...
    def rechercher(self):
        ...
        print("=== Débogage recherche ===")
        print("Mode de recherche :", mode)
        print("Prénom saisi :", repr(prenom))
        print("Nom saisi :", repr(nom))
        print("Vérification eleves_par_classes :", type(self.FrameDrBa.modifier_bdd.eleves_par_classes))

        for classe, liste_eleves in self.FrameDrBa.modifier_bdd.eleves_par_classes.items():
            print(f"\nClasse : {classe}")
            for eleve in liste_eleves:
                prenom_eleve = eleve[0].strip().lower()
                nom_eleve = eleve[1].strip().lower()
                print(f"→ Test avec élève : prénom={repr(prenom_eleve)}, nom={repr(nom_eleve)}")
        ...
        print("=== Fin recherche ===")
        print("Élèves trouvés :", self.FrameG.listeEleves)
    ...
    def information(self,event) -> None:
        ...
        self.fen_propos = tk.Toplevel()
        #bloque la fenêtre parente pour éviter de créer une deuxième fenêtre information        
        while 1:
            try:
                self.fen_propos.grab_set()
                break
            except tk.TclError:
                self.fen_propos.after(100) 
        self.fen_propos.title("À propos de: ")
        self.fen_propos.transient(self.fen_propos.master) # fenetre provisoire 
                            # qui ne sera pas afficher dans la barre de tâche
        frame1 = tk.Frame(self.fen_propos, padx=5, pady=1)
        frame1.pack()
        partie1=" Mémo_Lycée * v0.44 \n\n Copyrigth (c) - G LE REST - "
        ...
        # bouton
        bout_info = tk.Button(frame1, text="Quitter", 
                                    command=self.fen_propos.destroy)
        bout_info.pack(pady=2)
        #blocage du dimensionnement de la fenêtre
        self.fen_propos.resizable(width=False, height=False)
